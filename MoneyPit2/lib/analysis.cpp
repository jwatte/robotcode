...
void RPixmap::find_areas_of_color(Area area, Color c, float tolerance, float normalization, int min_areas,
    std::vector<ColorArea> &o_areas, bool fill, Color const &fillColor) {
    ...
    for (size_t i = 0, n = ret.size(); i != n; ++i) {
        ...
        if (fill) {
            frame_rect(Area(ca.area), fillColor);
        }
    }
...
size_t RPixmap::get_tga_header(void *data, size_t size) {
...
void RPixmap::frame_rect(Area const &a, Color c) {
    Area clip(a);
...
unsigned char *RPixmap::write_bits() {
...
void RPixmap::save_jpg(char const *name) {
    ...
    jpeg_set_quality(&cinfo, 210, false);
    jpeg_start_compress(&cinfo, TRUE);
    JSAMPROW row_pointer[1];
    while (cinfo.next_scanline < cinfo.image_height) {
        row_pointer[0] = (JSAMPROW)(bits + cinfo.next_scanline * rowbytes);
