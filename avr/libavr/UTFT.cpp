...
#include "UTFT.h"
// HACK: jwatte -- support avr-gcc without Arduino kit
...
template<byte model>
UTFT<model>::UTFT(int RS, int WR,int CS, int RST, int SER)
{ 
...
template<byte model>
void UTFT<model>::LCD_Write_COM(char VL)  
{   
	if (display_transfer_mode!=1)
	{
		cbi(P_RS, B_RS);
...
template<byte model>
void UTFT<model>::LCD_Write_DATA(char VH,char VL)
{
	if (display_transfer_mode!=1)
	{
		sbi(P_RS, B_RS);
...
template<byte model>
void UTFT<model>::LCD_Write_DATA(char VL)
{
	if (display_transfer_mode!=1)
	{
		sbi(P_RS, B_RS);
...
template<byte model>
void UTFT<model>::LCD_Write_COM_DATA(char com1,int dat1)
{
     LCD_Write_COM(com1);
...
template<byte model>
void UTFT<model>::InitLCD(byte orientation)
{
	orient=orientation;
...
template<byte model>
void UTFT<model>::setXY(word x1, word y1, word x2, word y2)
{
	if (orient==LANDSCAPE)
	{
		swap(word, x1, y1);
...
template<byte model>
void UTFT<model>::drawRect(int x1, int y1, int x2, int y2)
{
	if (x1>x2)
	{
		swap(int, x1, x2);
...
template<byte model>
void UTFT<model>::drawRoundRect(int x1, int y1, int x2, int y2)
{
	if (x1>x2)
	{
		swap(int, x1, x2);
...
template<byte model>
void UTFT<model>::fillRect(int x1, int y1, int x2, int y2)
{
	if (x1>x2)
	{
		swap(int, x1, x2);
...
template<byte model>
void UTFT<model>::fillRoundRect(int x1, int y1, int x2, int y2)
{
	if (x1>x2)
	{
		swap(int, x1, x2);
...
template<byte model>
void UTFT<model>::drawCircle(int x, int y, int radius)
{
	int f = 1 - radius;
	int ddF_x = 1;
	int ddF_y = -2 * radius;
...
template<byte model>
void UTFT<model>::fillCircle(int x, int y, int radius)
{
	cbi(P_CS, B_CS);
...
template<byte model>
void UTFT<model>::fillScr(byte r, byte g, byte b)
{
	long i;
	char ch, cl;
...
template<byte model>
void UTFT<model>::setColor(byte r, byte g, byte b)
{
	fcolorr=r;
	fcolorg=g;
	fcolorb=b;
...
template<byte model>
void UTFT<model>::setBackColor(byte r, byte g, byte b)
{
	bcolorr=r;
	bcolorg=g;
	bcolorb=b;
...
template<byte model>
void UTFT<model>::setPixel(byte r,byte g,byte b)
{
	LCD_Write_DATA(((r&248)|g>>5),((g&28)<<3|b>>3));	// rrrrrggggggbbbbb
}

template<byte model>
void UTFT<model>::drawPixel(int x, int y)
{
	cbi(P_CS, B_CS);
	setXY(x, y, x, y);
...
template<byte model>
void UTFT<model>::drawLine(int x1, int y1, int x2, int y2)
{
	double delta, tx, ty;
	char ch, cl;
...
template<byte model>
void UTFT<model>::drawHLine(int x, int y, int l)
{
	char ch, cl;
...
template<byte model>
void UTFT<model>::drawVLine(int x, int y, int l)
{
	char ch, cl;
...
template<byte model>
void UTFT<model>::printChar(byte c, int x, int y)
{
	byte i,ch;
	word j;
	word temp; 
...
template<byte model>
void UTFT<model>::rotateChar(byte c, int x, int y, int pos, int deg)
{
	byte i,j,ch;
	word temp; 
	int newx,newy;
...
template<byte model>
void UTFT<model>::print(char const *st, int x, int y, int deg)
{
	int stl, i;
...
/*
template<byte model>
void UTFT<model>::print(String st, int x, int y, int deg)
{
...
template<byte model>
void UTFT<model>::printNumI(long num, int x, int y, int length, char filler)
{
	char buf[25];
	char st[27];
	boolean neg=false;
...
template<byte model>
void UTFT<model>::printNumF(double num, byte dec, int x, int y, char divider, int length, char filler)
{
	char buf[25];
	char st[27];
	boolean neg=false;
...
template<byte model>
void UTFT<model>::drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int scale)
{
	unsigned int col;
...
template<byte model>
void UTFT<model>::drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int deg, int rox, int roy)
{
	unsigned int col;
	int tx, ty, newx, newy;
	double radian;
...
template<byte model>
void UTFT<model>::setContrast(char c)
{
	cbi(P_CS, B_CS);
	switch (display_model)
	{
...
template<byte model>
int UTFT<model>::getDisplayXSize()
{
	if (orient==PORTRAIT)
...
template<byte model>
int UTFT<model>::getDisplayYSize()
{
	if (orient==PORTRAIT)
		return disp_y_size+1;
	else
